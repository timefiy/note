Create a single horizontal sprite strip for the Codex app digital pet `pikachu` in the state `waving`.
...
Output exactly 4 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Pikachu, yellow electric mouse mascot, red cheeks, black-tipped ears, lightning-bolt tail, codex digital pet style, compact chibi, thick dark outline, limited palette, flat cel shading.
