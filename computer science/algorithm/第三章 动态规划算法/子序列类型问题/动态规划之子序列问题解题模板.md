...
published:
created: 2026-02-17
description: 本文对子序列系列问题进行归纳，给出两种动态规划思路模板，并给出代码实现。
tags:
  - clippings
---
...
而且，子序列问题很可能涉及到两个字符串，比如前文 [最长公共子序列](https://labuladong.online/zh/algo/dynamic-programming/longest-common-subsequence/) ，如果没有一定的处理经验，真的不容易想出来。所以本文就来扒一扒子序列问题的套路，其实就有两种模板，相关问题只要往这两种思路上想，十拿九稳。
...
比如我们写过的 [最长递增子序列](https://labuladong.online/zh/algo/dynamic-programming/longest-increasing-subsequence/) 和 [最大子数组和](https://labuladong.online/zh/algo/dynamic-programming/maximum-subarray/) 都是这个思路。
...
这种思路运用相对更多一些，尤其是涉及两个字符串/数组的子序列时，比如前文讲的 [最长公共子序列](https://labuladong.online/zh/algo/dynamic-programming/longest-common-subsequence/) 和 [编辑距离](https://labuladong.online/zh/algo/dynamic-programming/edit-distance/) ；这种思路也可以用于只涉及一个字符串/数组的情景，比如本文讲的回文子序列问题。
...
之前解决了 [最长回文子串](https://labuladong.online/zh/algo/essential-technique/array-two-pointers-summary/) 的问题，这次提升难度，看看力扣第 516 题「 [最长回文子序列](https://leetcode.cn/problems/longest-palindromic-subsequence/) 」，求最长回文子序列的长度：
...
为啥这个问题要这样定义二维的 `dp` 数组呢？我在 [最长递增子序列](https://labuladong.online/zh/algo/dynamic-programming/longest-increasing-subsequence/) 提到，找状态转移需要归纳思维，说白了就是如何从已知的结果推出未知的部分。而这样定义能够进行归纳，容易发现状态转移关系。

具体来说，如果我们想求 `dp[i][j]` ，假设你知道了子问题 `dp[i+1][j-1]` 的结果（ `s[i+1..j-1]` 中最长回文子序列的长度），你是否能想办法算出 `dp[i][j]` 的值（ `s[i..j]` 中，最长回文子序列的长度）呢？

![](https://labuladong.online/images/algo/lps/1.jpg)

可以！这取决于 `s[i]` 和 `s[j]` 的字符：

**如果它俩相等** ，那么它俩加上 `s[i+1..j-1]` 中的最长回文子序列就是 `s[i..j]` 的最长回文子序列：

![](https://labuladong.online/images/algo/lps/2.jpg)

**如果它俩不相等** ，说明它俩 **不可能同时** 出现在 `s[i..j]` 的最长回文子序列中，那么把它俩 **分别** 加入 `s[i+1..j-1]` 中，看看哪个子串产生的回文子序列更长即可：

![](https://labuladong.online/images/algo/lps/3.jpg)
...
关于 `dp` 数组的遍历方向，详情见 [动态规划答疑篇](https://labuladong.online/zh/algo/dynamic-programming/faq-summary/) 。
...
![](https://labuladong.online/images/algo/palindrome-insert/1.jpeg)
