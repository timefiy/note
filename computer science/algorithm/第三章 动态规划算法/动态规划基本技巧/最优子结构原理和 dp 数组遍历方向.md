---
title: 最优子结构原理和 dp 数组遍历方向
source: https://labuladong.online/zh/algo/dynamic-programming/faq-summary/
author:
  - "[[../../../../author/labuladong]]"
published:
created: 2026-02-17
description: 本文讲解动态规划的常见问题，包括什么是最优子结构、如何快速判断是否存在重叠子问题、如何设置 dp 数组的遍历方向、如何初始化 dp 数组的大小等。
tags:
  - clippings
---
前置知识
...
本文作为 [动态规划核心套路框架](https://labuladong.online/zh/algo/essential-technique/dynamic-programming-framework/) 之后的一篇全面答疑文章，给你讲明白以下几个问题：
...
这次我给你提出的问题就 **不符合最优子结构** ，因为你没办通过每个班的最优值推出全校的最优值，没办法通过子问题的最优值推出规模更大的问题的最优值。前文 [动态规划详解](https://labuladong.online/zh/algo/essential-technique/dynamic-programming-framework/) 说过，想满足最优子结，子问题之间必须互相独立。全校的最大分数差可能出现在两个班之间，显然子问题不独立，所以这个问题本身不符合最优子结构。
...
前文 [高楼扔鸡蛋问题](https://labuladong.online/zh/algo/dynamic-programming/egg-drop/) 就展示了如何改造问题，不同的最优子结构，可能导致不同的解法和效率。
...
看了前文 [动态规划核心套路](https://labuladong.online/zh/algo/essential-technique/dynamic-programming-framework/) ，我知道了如何一步步优化动态规划问题；

看了前文 [动态规划设计：数学归纳法](https://labuladong.online/zh/algo/dynamic-programming/longest-increasing-subsequence/) ，我知道了利用数学归纳法写出暴力解（状态转移方程）。
...
比如最简单的例子， [动态规划核心套路](https://labuladong.online/zh/algo/essential-technique/dynamic-programming-framework/) 中斐波那契数列的递归树：

![](https://labuladong.online/images/algo/dynamic-programming/1.jpg)

这棵递归树很明显存在重复的节点，所以我们可以通过备忘录避免冗余计算。

但毕竟斐波那契数列问题太简单了，实际的动态规划问题比较复杂，比如二维甚至三维的动态规划，当然也可以画递归树，但不免有些复杂。

比如在 [最小路径和问题](https://labuladong.online/zh/algo/dynamic-programming/minimum-path-sum/) 中，我们写出了这样一个暴力解法：
...
![](https://labuladong.online/images/algo/optimal/2.jpeg)
...
再举个稍微复杂的例子，前文 [正则表达式问题](https://labuladong.online/zh/algo/dynamic-programming/regular-expression-matching/) 的暴力解代码：
...
比如说前文 [编辑距离问题](https://labuladong.online/zh/algo/dynamic-programming/edit-distance/) ，我首先讲的是自顶向下的递归解法，实现了这样一个 `dp` 函数：
...
甚至更让人迷惑的是，有时候发现正向反向遍历都可以得到正确答案，比如我们在 [团灭股票问题](https://labuladong.online/zh/algo/dynamic-programming/stock-problem-summary/) 中有的地方就正反皆可。
...
比如 [编辑距离](https://labuladong.online/zh/algo/dynamic-programming/edit-distance/) 这个经典的问题，我们通过对 `dp` 数组的定义，确定了 base case 是 `dp[..][0]` 和 `dp[0][..]` ，最终答案是 `dp[m][n]` ；而且我们通过状态转移方程知道 `dp[i][j]` 需要从 `dp[i-1][j]`, `dp[i][j-1]`, `dp[i-1][j-1]` 转移而来，如下图：

![](https://labuladong.online/images/algo/optimal/1.jpg)
...
再举一例，回文子序列问题，详见前文 [子序列问题模板](https://labuladong.online/zh/algo/dynamic-programming/subsequence-problem/) ，我们通过过对 `dp` 数组的定义，确定了 base case 处在中间的对角线， `dp[i][j]` 需要从 `dp[i+1][j]`, `dp[i][j-1]`, `dp[i+1][j-1]` 转移而来，想要求的最终答案是 `dp[0][n-1]` ，如下图：

![](https://labuladong.online/images/algo/lps/4.jpg)

这种情况根据刚才的两个原则，就可以有两种正确的遍历方式：

![](https://labuladong.online/images/algo/lps/5.jpg)
