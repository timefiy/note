...
- 切片（slice）是对一段连续数据的借用视图，本身不拥有数据所有权。
- 它只保存“起始位置 + 长度”信息，因此创建切片通常是轻量操作。
- 切片必须依附于原始数据存在，原始数据失效时切片也会失效。
- 常见形式：
  - 字符串切片：`&str`
  - 数组切片：`&[T]`
  - 可变切片：`&mut [T]`
...
- **字符串字面值**（String Literal）的数据被直接硬编码在编译后的二进制文件中。当我们将其赋值给变量时，该变量本质上是一个具有静态生命周期的切片（Slice）视图。
```rust
let s = "hello world";
// s的类型是&str，执向二进制程序特定位置的切片
```

| **特性**   | **字符串字面值**                    | **动态字符串 (String)**            |
| -------- | ----------------------------------------- | ----------------------------- |
...
*注意*
.rodata 是受操作系统硬件级保护的绝对只读段，这就注定了：可变切片（&mut str 或` &mut [u8]`）绝对不可能指向静态的二进制文件数据。  
如果代码中需要定义或使用可变切片，其底层指向的数据必须存在于允许读写的内存区域，即 栈内存 (Stack) 或 堆内存 (Heap)。 
