...
published:
created: 2026-02-17
description: 本文讲解如何利用球盒模型写出回溯算法解决，解决力扣/LeetCode 上的 698 题，同时给出 Java/Python/Go/JavaScript/C++ 代码实现。
tags:
  - clippings
---
...
我们之前 [背包问题之子集划分](https://labuladong.online/zh/algo/dynamic-programming/knapsack2/) 写过一次子集划分问题，不过那道题只需要我们把集合划分成两个相等的集合，可以转化成背包问题用动态规划技巧解决。
...
[背包问题之子集划分](https://labuladong.online/zh/algo/dynamic-programming/knapsack2/) 的场景中，有一个背包和若干物品，每个物品有 **两个选择** ，分别是「装进背包」和「不装进背包」。把原集合 `S` 划分成两个相等子集 `S_1, S_2` 的场景下， `S` 中的每个元素也有 **两个选择** ，分别是「装进 `S_1` 」和「不装进 `S_1` （装进 `S_2` ）」，这时候的穷举思路其实和背包问题相同。
...
前文 [用球盒模型理解回溯算法](https://labuladong.online/zh/algo/practice-in-action/two-views-of-backtrack/) 说过，回溯算法的关键在哪里？
...
![](https://labuladong.online/images/algo/set-split/5.jpeg)

**视角二，如果我们切换到这 `k` 个桶的视角，对于每个桶，都要遍历 `nums` 中的 `n` 个数字，然后选择是否将当前遍历到的数字装进自己这个桶里** 。

![](https://labuladong.online/images/algo/set-split/6.jpeg)
...
不要被这么多参数吓到，我会一个个解释这些参数。如果你够透彻理解了前文 [用球盒模型理解回溯算法](https://labuladong.online/zh/algo/practice-in-action/two-views-of-backtrack/) ，也能得心应手地写出这样的回溯函数。
...
![](https://labuladong.online/images/algo/set-split/1.jpeg)

现在第一个桶装满了，就开始装第二个桶，算法会装入 `2, 3` ：

![](https://labuladong.online/images/algo/set-split/2.jpeg)
...
![](https://labuladong.online/images/algo/set-split/3.jpeg)

现在第一个桶装满了，就开始装第二个桶，算法会装入 `1, 4` ：

![](https://labuladong.online/images/algo/set-split/4.jpeg)
