...
published:
created: 2026-02-17
description: 本文讲解戳气球问题的动态规划解法，运用动态规划解题的标准流程推导状态转移方程并给出代码实现。
tags:
  - clippings
---
...
今天我们要聊的这道题「Burst Balloon」和之前我们写过的那篇 [经典动态规划：高楼扔鸡蛋问题](https://labuladong.online/zh/algo/dynamic-programming/egg-drop/) 分析过的高楼扔鸡蛋问题类似，知名度比较高，但难度确实也不小。
...
那么，这不就是一个「全排列」问题嘛，我们前文 [回溯算法框架套路详解](https://labuladong.online/zh/algo/essential-technique/backtrack-framework/) 中有全排列算法的详解和代码，其实只要稍微改一下逻辑即可，伪码思路如下：
...
[动态规划套路框架详解](https://labuladong.online/zh/algo/essential-technique/dynamic-programming-framework/) 说过运用动态规划算法的一个重要条件： **子问题必须独立** 。所以对于这个戳气球问题，如果想用动态规划，必须巧妙地定义 `dp` 数组的含义，避免子问题产生相关性，才能推出合理的状态转移方程。
...
关于 `dp` 函数/数组的定义，大部分题目都比较容易想到，少数题目需要巧妙的定义，一定程度上依赖经验和灵感，这道题就属于后者，所以它的难度级别是困难。

对于这道题，可以这样思考：

首先要进行归一化，简化问题的处理。因为本题中计算戳气球的得分时，可能有三个气球、两个气球、一个气球好几种情况。我们如果添加一个虚拟的边界，就可以把这些情况都统一成三个气球的情况。

有了这一个简单的转化，问题结构就清楚了。现在就是让你随意戳气球，戳破一个气球，得分就是这个气球和两个相邻气球的乘积。如果小于三个气球，就停止戳气球。

这样就比较容易想到定义 `dp[i][j]` 为整个开区间中的最大得分，尝试写状态转移方程了。

```
// 定义：dp[i][j] 表示戳破气球 i 和气球 j 之间（开区间）的所有气球能得到的最高分数
...
![](https://labuladong.online/images/algo/burstBalloon/1.jpeg)
...
肯定也有读者要问了，这个状态转移看起来很巧妙，未来在做新题的时候，如何才能靠自己想到这种巧妙的状态转移呢？

其实这个状态转移并没有多巧妙，只是根据你的 `dp` 数组定义推导出来的罢了。

因为你刚才定义了 `dp[i][j]` 表示戳破气球 `i` 和气球 `j` 之间的所有气球能得到的最高分数，那么现在就是要思考，如何通过子问题推导出原问题。

具体来说，你应该思考，你需要知道哪些子问题的答案，才能推导出原问题 `dp[i][j]` 的答案呢？

那么就很自然地想到，对于一个较大的区间，就好比 `(4, 10)` 这个区间，是可以分解为若干子区间的，比如 `(4, 6) + (6, 10) + 6` 。再结合 `dp` 数组定义，就会发现这样分解恰好可以利用子问题的答案来推导出原问题的答案。
...
那么应该如何安排 `i` 和 `j` 的遍历顺序，来提供上述的保证呢？我们前文 [动态规划答疑篇](https://labuladong.online/zh/algo/dynamic-programming/faq-summary/) 写过处理这种问题的一个鸡贼技巧： **根据 base case 和最终状态进行推导** 。
...
![](https://labuladong.online/images/algo/burstBalloon/2.jpeg)

对于任一 `dp[i][j]` ，我们希望所有 `dp[i][k]` 和 `dp[k][j]` 已经被计算，画在图上就是这种情况：

![](https://labuladong.online/images/algo/burstBalloon/3.jpeg)

那么，为了达到这个要求，可以有两种遍历方法，要么斜着遍历，要么从下到上从左到右遍历：

![](https://labuladong.online/images/algo/burstBalloon/4.jpeg)

![](https://labuladong.online/images/algo/burstBalloon/5.jpeg)
