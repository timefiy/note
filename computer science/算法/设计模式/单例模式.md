...
为了保证唯一性，代码通过以下手段让外部无法通过常规方式创建 `DatabaseManager`：

- **`private` 构造函数**：`DatabaseManager()` 被设为私有。这意味着你不能在类外部写 `DatabaseManager db;`，否则编译器会报错。
    
- **`= delete` 拷贝控制**：
    
    - `DatabaseManager(const DatabaseManager&) = delete;`
        
    - `DatabaseManager& operator=(const DatabaseManager&) = delete;` 这两个操作阻止了通过“拷贝”现有对象来产生新对象的行为（例如 `auto db2 = DatabaseManager::getInstance();` 会失效）。
        

#### 2. 自产：自己创建并持有唯一实例

- **`static DatabaseManager instance;`**：类内部定义了一个静态成员。静态成员属于类本身，而不属于某个具体的对象。
    
- **静态成员定义**：在类外部的 `DatabaseManager DatabaseManager::instance;` 负责真正的内存分配。
    
    - **饿汉式特点**：由于它是全局静态变量，程序一启动（进入 `main` 函数之前），这个实例就已经创建好并完成了数据库连接。
...
- **`getInstance()` 方法**：
    ...
    - 它是 `static` 的，意味着不需要对象就能调用。
        
    - 它返回 `instance` 的引用。无论你在程序的哪个角落调用 `DatabaseManager::getInstance()`，拿到的永远是内存中同一个位置的那个对象。
### 2\. 懒汉式实现（双重检查锁定）
...
- **线程安全**：`getInstance()` 方法中需要使用同步锁防止多线程同时进入造成实例被多次创建。
- **延迟初始化**：懒汉式实例在第一次调用 `getInstance()` 方法时创建。
