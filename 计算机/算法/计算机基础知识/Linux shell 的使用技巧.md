...
前文 [Linux文件描述符](https://labuladong.online/zh/algo/other-skills/linux-process/) 说过，管道符和重定向符是将数据作为程序的标准输入，而 `$(cmd)` 是读取 `cmd` 命令输出的数据作为参数。
...
![](https://labuladong.online/images/algo/linuxshell/1.png)
...
如果你读过前文 [Linux 文件描述符](https://labuladong.online/zh/algo/other-skills/linux-process/) ，就知道其原理很简单：
