...
published:
created: 2026-02-06
description: 本文讲解的线段树基本原理及应用场景，线段树是二叉树结构的延伸，用于高效解决区间的动态修改和查询问题。
tags:
  - clippings
  - 线段树
  - 笔记
...
- [二叉树的递归层序遍历](../二叉树结构及遍历/二叉树的递归层序遍历.md)
...
上面的实现都只支持「单点更新」，但更通用的需求是区间更新，比如把索引区间 `[i, j]` 的元素都更新为 `val` 。 [懒更新线段树的实现](https://labuladong.online/zh/algo/data-structure/segment-tree-lazy-update/) 运用「懒更新」技巧，给线段树新增 `rangeAdd/rangeUpdate` 方法，可以在 $O(\log N)$ 时间复杂度内完成 **任意长度** 的区间更新。
...
后面的章节你就会学到，这种技巧可以统一归为 [前缀和技巧](https://labuladong.online/zh/algo/data-structure/prefix-sum/) ，主要用于处理区间查询问题。
...
下面的可视化面板创建了一个求区间和的线段树，请你注意树的结构，这是一棵 [高度平衡二叉树](https://labuladong.online/zh/algo/data-structure-basic/binary-tree-basic/) ，其中叶子节点是 `nums` 数组的元素，非叶子节点记录着区间 `[i, j]` 的元素和：
...
综上， `query` 方法遍历的节点总数是 $\log N$ 的常数倍，所以时间复杂度是 $O(\log N)$ 。
...
上面就是线段树结构的核心原理，具体的实现及优化将安排在后面的数据结构设计章节 [基本线段树的代码实现](https://labuladong.online/zh/algo/data-structure/segment-tree-implement/) ，那时候你应该可以熟练地操作和构造二叉树了，所以线段树的代码会比较容易理解。
