...
published:
created: 2026-02-17
description: 本文讲解动态规划的空间压缩技巧，可以把二维的 dp 数组转化成一维，从而降低空间复杂度，优化效率。
tags:
  - clippings
---
前置知识
...
什么叫「和 `dp[i][j]` 相邻的状态」呢，比如前文 [最长回文子序列](https://labuladong.online/zh/algo/dynamic-programming/subsequence-problem/) 中，最终的代码如下：
...
![](https://labuladong.online/images/algo/space-optimal/1.jpeg)

这就叫和 `dp[i][j]` 相邻，反正你计算 `dp[i][j]` 只需要这三个相邻状态，其实根本不需要那么大一个二维的 dp table 对不对？ **空间压缩的核心思路就是，将二维数组「投影」到一维数组** ：

![](https://labuladong.online/images/algo/space-optimal/2.jpeg)
...
我们先简单进行一下 **滚动数组优化** 。既然 `dp[i][j]` 只依赖于 `dp[i+1][..]` 和 `dp[i][..]` ，那么我们其实不需要维护整个 `N * N` 的 `dp` 数组，只需要维护两行 `dp` 数组即可。

我们可以使用 `dp[2][n]` 这样的数组，利用取模运算 `% 2` 来交替使用这两行：
...
**从实用性的角度来说，优化到这里就够了，因为这个只有两行的 `dp` 数组空间复杂度已经降到 `O(N)` 了** ，几乎等同于一维数组了。

下面更进一步，把 `dp` 数组直接压缩成一维数组。
...
![](https://labuladong.online/images/algo/space-optimal/3.jpeg)
...
![](https://labuladong.online/images/algo/space-optimal/4.jpeg)
...
也就是说，你最起码能够熟练运用我们前文 [动态规划框架套路详解](https://labuladong.online/zh/algo/essential-technique/dynamic-programming-framework/) 的套路找出状态转移方程，写出一个正确的动态规划解法，然后才有可能观察状态转移的情况，分析是否可能使用空间压缩技巧来优化。
