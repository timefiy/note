...
published:
created: 2026-02-17
description: 本文讲解 力扣/LeetCode 645 题错误的集合，运用数学技巧和位运算逐步求解，并给出 Java/Python/Go/JavaScript/C++ 代码实现。
tags:
  - clippings
---
...
今天就聊一道很看起来简单却十分巧妙的问题，寻找缺失和重复的元素。之前的一篇文章 [常用的位操作](https://labuladong.online/zh/algo/frequency-interview/bitwise-operation/) 中也写过类似的问题，不过这次的和上次的问题使用的技巧不同。
...
![](https://labuladong.online/images/algo/dupmissing/1.gif)

如果出现重复元素 `4` ，直观结果就是，索引 `4` 所对应的元素已经是负数了：

![](https://labuladong.online/images/algo/dupmissing/2.jpg)

对于缺失元素 `3` ，直观结果就是，索引 `3` 所对应的元素是正数：

![](https://labuladong.online/images/algo/dupmissing/3.jpg)
...
异或运算也是常用的，因为异或性质 `a ^ a = 0, a ^ 0 = a` ，如果将索引和元素同时异或，就可以消除成对儿的索引和元素，留下的就是重复或者缺失的元素。可以看看前文 [常用的位运算](https://labuladong.online/zh/algo/frequency-interview/bitwise-operation/) ，介绍过这种方法。
