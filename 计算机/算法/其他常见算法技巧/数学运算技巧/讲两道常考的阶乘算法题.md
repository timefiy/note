...
published:
created: 2026-02-17
description: 本文讲解 力扣/LeetCode 上两道阶乘系列题目的解法，关键在于寻找数学规律并运用二分搜索技巧，最后给出 Java/Python/Go/JavaScript/C++ 代码实现。
tags:
  - clippings
---
...
前文 [二分查找如何运用](https://labuladong.online/zh/algo/frequency-interview/binary-search-in-action/) 说过， **对于这种具有单调性的函数，用 for 循环遍历，可以用二分查找进行降维打击** ，对吧？

搜索有多少个 `n` 满足 `trailingZeroes(n) == K` ，其实就是在问， **满足条件的 `n` 最小是多少，最大是多少，最大值和最小值一减，就可以算出来有多少个 `n` 满足条件了** ，对吧？那不就是 [二分查找](https://labuladong.online/zh/algo/essential-technique/binary-search-framework/) 中「搜索左侧边界」和「搜索右侧边界」这两个事儿嘛？

先不急写代码，因为二分查找需要给一个搜索区间，也就是上界和下界，上述伪码中 `n` 的下界显然是 0，但上界是 `+inf` ，这个正无穷应该如何表示出来呢？

首先，数学上的正无穷肯定是无法编程表示出来的，我们一般的方法是用一个非常大的值，大到这个值一定不会被取到。比如说 int 类型的最大值 `INT_MAX` （2^31 - 1，大约 21 亿），还不够的话就 long 类型的最大值 `LONG_MAX` （2^63 - 1，这个值就大到离谱了）。
...
根据前文 [二分查找算法框架](https://labuladong.online/zh/algo/essential-technique/binary-search-framework/) ，可以直接把搜索左侧边界和右侧边界的框架 copy 过来。注意我使用的是左闭右开的搜索区间：
...
如果对二分查找的框架有任何疑问，建议好好复习一下前文 [二分查找算法框架](https://labuladong.online/zh/algo/essential-technique/binary-search-framework/) ，这里就不展开了。
