...
published:
created: 2026-02-17
description: 本文结合 力扣/LeetCode 的例题讲解随机算法，包括水塘抽样算法 (Reservoir Sampling) 、洗牌算法 (Shuffle Algorithm) 和蒙特卡洛方法 (Monte Carlo Method) ，并给出 Java/Python/Go/JavaScript/C++ 代码实现。
tags:
  - clippings
---
...
![](https://labuladong.online/images/algo/random-algo/1.jpg)

再比如经典炸弹人游戏，障碍物的位置也是有一定随机性的：

![](https://labuladong.online/images/algo/random-algo/2.jpg)
...
**384\. 打乱数组** | [力扣](https://leetcode.cn/problems/shuffle-an-array/) | [LeetCode](https://leetcode.com/problems/shuffle-an-array/)
...
题目来源： [力扣 384. 打乱数组](https://leetcode.cn/problems/shuffle-an-array/) 。
...
![](https://labuladong.online/images/algo/sample/formula1.png)
...
这个思路可以解决力扣第 398 题「 [随机数索引](https://leetcode.cn/problems/random-pick-index/) 」。对于算法正确性的数学证明，和上面区别不大：

![](https://labuladong.online/images/algo/sample/formula2.png)
...
vector<int> sample(int lo, int hi, int k) {
...
上面讲到的洗牌算法和水塘抽样算法都属于随机概率算法，虽然从数学推导上可以证明算法的思路是正确的，但如果你笔误写出 bug，就会导致概率上的不均等。更神奇的是，力扣的判题机制能够检测出这种概率错误，你猜它是怎么做到的？

最后来介绍一种检测随机算法正确性的方法：蒙特卡洛方法。我猜测力扣的判题系统就是利用这个方法来判断随机算法的正确性的。

记得高中有道数学题：往一个正方形里面随机打点，这个正方形里紧贴着一个圆，告诉你打点的总数和落在圆里的点的数量，让你计算圆周率。

![](https://labuladong.online/images/algo/shuffle/4.png)
...
int main() {
    ...
    for (int i = 0; i < N; i++) {
        ...
        std::vector<int> res = sample(lo, hi, k);
