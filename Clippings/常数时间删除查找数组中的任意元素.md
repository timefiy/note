---
title: "常数时间删除/查找数组中的任意元素"
source: "https://labuladong.online/zh/algo/data-structure/random-set/"
author:
  - "[[labuladong]]"
published:
created: 2026-02-17
description: "本文利用哈希表，支持在数组中常数时间插入、删除和随机获取元素，同时给出 Java/Python/Go/JavaScript/C++ 代码实现。"
tags:
  - "clippings"
---
...
本文讲两道比较有技巧性的数据结构设计题，都是和随机读取元素相关的，我在前文 [谈谈游戏中的随机算法](https://labuladong.online/zh/algo/frequency-interview/random-algorithm/) 也写过类似的问题。
...
题目来源： [力扣 380. O(1) 时间插入、删除和获取随机元素](https://leetcode.cn/problems/insert-delete-getrandom-o1/) 。
...
public:
    ...
    // 如果 val 不存在集合中，则插入并返回 true，否则直接返回
...
除了 `HashSet` ，还有一些类似的数据结构，比如 [哈希链表 `LinkedHashSet`](https://labuladong.online/zh/algo/data-structure-basic/hashtable-with-linked-list/) ，这类数据结构的实现原理本质上就是哈希表配合双链表，元素存储在双链表中。

但是， `LinkedHashSet` 只是给 `HashSet` 增加了插入的先后顺序，依然无法按要求实现我们的 `getRandom` 函数，因为底层用链表结构存储元素的话，是无法在 O(1) 的时间内访问某一个元素的。
...
其实本站开头的 [用数组加强哈希表（ArrayHashMap）](https://labuladong.online/zh/algo/data-structure-basic/hashtable-with-array/) 探讨的就是这个问题，直接看代码吧：
...
int pick() {
    ...
    int res = randomPick(0..N-1);

    while (res exists in blacklist) {

        // 重新随机一个结果

        res = randomPick(0..N-1);
...
这个函数会多次生成随机数，执行效率竟然是随机的，不是一个聪明的解法。

**聪明的解法类似上一道题，我们可以将区间 `[0,N)` 看做一个数组，然后将 `blacklist` 中的元素移到数组的最末尾，同时用一个哈希表进行映射**.

当然，这道题的难点在于，只给你输入了一个数字 `N` ，并不是一个真实的数组，所以我们需要借助哈希表操作来模拟「把黑名单数字移到数组末尾」的效果。
...
public:
    ...
    Solution(int N, vector<int>& blacklist) {
        ...
        distribution = uniform_int_distribution<int>(0, sz - 1);
...
![](https://labuladong.online/images/algo/random-set/1.jpeg)
...
我们将黑名单中的元素 `b` 映射到元素 `last` ，但是我们能确定元素 `last` 就不在 `blacklist` 中吗？

比如下图这种情况，我们的预期应该是 1 映射到 3，但是错误地映射到 4：

![](https://labuladong.online/images/algo/random-set/2.jpeg)

**所以，在对 `mapping[b]` 赋值时，要保证 `last` 一定不在 `blacklist` 中** ，可以如下操作：
...
![](https://labuladong.online/images/algo/random-set/3.jpeg)
...
本文就到这里，更多数据结构设计相关的题目参见 [数据结构设计经典习题](https://labuladong.online/zh/algo/problem-set/ds-design/) 。
