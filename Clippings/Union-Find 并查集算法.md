...
- [多叉树基础及遍历](https://labuladong.online/zh/algo/data-structure-basic/n-ary-tree-traverse-basic/)
...
而且，因为我们记录了每棵树的重量，所以还添加了一个 `size(x)` 方法快速获取 `x` 所在的连通分量的节点总数。

此时， `find, union, connected, size` 的时间复杂度都下降为 $O(logN)$ ，即便数据规模上亿，所需时间也非常少。
...
我曾经认为这种递归写法和第一种迭代写法做的事情一样，但实际上是我大意了，有读者指出这种写法进行路径压缩的效率是高于上一种解法的。
...
比起第一种路径压缩，显然这种方法压缩得更彻底，直接把一整条树枝压平，一点意外都没有。就算一些极端情况下产生了一棵比较高的树，只要一次路径压缩就能大幅降低树高，从 [摊还分析](https://labuladong.online/zh/algo/essential-technique/complexity-analysis/) 的角度来看，所有操作的平均时间复杂度依然是 O(1)，所以从效率的角度来说，推荐你使用这种路径压缩算法。
...
不过，对于有些题目，我们即便使用了路径压缩技巧，可能还需要保留 `size` 数组，这样可以帮助我们计算每个联通分量的节点数量。

比方说，让你实现一个 `size(x)` 方法，返回节点 `x` 所在连通分量的节点总数，那么就只能借助 `size` 数组来实现，否则就需要遍历整棵树：
...
注意，我们的 `size(x)` 方法是先获取 `x` 的根节点，根节点记录的节点总数才是联通分量的节点总数。
